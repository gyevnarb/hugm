...
namespace core
{
    public class RUtils
    {
        ...
        public async Task<Graph> GenerateMarkovAnalysis(Graph g, string selection, int nsims = 100, int ndists = 18, double popcons = 0.15, int seed = 1,
            int nloop = 1, double beta = 2500.0, double eprob = 0.01, double lambda = 0.0, string savePath = "partitions.csv")
        {
            ...
            await Task.Run(() =>
            {
                try
                {
                    string run_analysis = File.ReadAllText("rcode/markov_analysis.R");
            ...
            return AreaUtils.Load(graphPath);
        }
